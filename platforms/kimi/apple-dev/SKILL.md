...
1. Run a **Debug build on a real device signed into iCloud** (a simulator with no iCloud account won't generate the schema), and exercise the code paths that write each record type. Or call `initializeCloudKitSchema(options:)` once inside `#if DEBUG` to force-generate every record type without exercising each path by hand — **never ship that call**, it fails against Production.
